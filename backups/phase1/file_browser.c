...
static const char* g_imageExtensions[] = {
    ".jpg", ".jpeg", ".png", ".bmp", ".gif", ".tga", ".psd", ".hdr", ".pic", ".pnm", NULL
};
...
int FileBrowser_OpenDialog(FileBrowser* browser, HWND hwnd) {
    ...
    ofn.hwndOwner = hwnd;
    ofn.lpstrFilter = "Image Files\0*.jpg;*.jpeg;*.png;*.bmp;*.gif;*.tga;*.psd;*.hdr\0"
                      "All Files\0*.*\0";
            ...
            if (!(findData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)) {
                if (FileBrowser_IsImageFile(findData.cFileName)) {
                    if (browser->fileCount < MAX_FILES) {
                        snprintf(browser->files[browser->fileCount], MAX_PATH, 
                                 "%s\\%s", dir, findData.cFileName);
                        browser->fileCount++;
                    }
                }
            }
...
const char* FileBrowser_GetCurrent(FileBrowser* browser) {
    if (browser->currentIndex >= 0 && browser->currentIndex < browser->fileCount) {
...
const char* FileBrowser_Next(FileBrowser* browser) {
    if (browser->fileCount == 0) return NULL;
